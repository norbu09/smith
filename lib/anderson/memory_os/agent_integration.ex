defmodule Anderson.MemoryOS.AgentIntegration do
  @moduledoc """
  ...
  def initialize_agent(agent_id, agent_config \\ %{}) do
    ...
    with {:ok, config} <- Anderson.MemoryOS.Configuration.get_or_create(agent_id, "default", agent_config),
  ...
  def trigger_memory_maintenance(agent_id, maintenance_options \\ %{}) do
    ...
    operations = maintenance_options[:operations] || [:heat_update, :capacity_check, :lpm_evaluation]
    results = []

    # Schedule immediate background jobs
    results = if :heat_update in operations do
    ...
    results = if :capacity_check in operations do
    ...
    results = if :lpm_evaluation in operations do
  ...
  def search_memories(agent_id, search_query, search_options \\ %{}) do
    Logger.debug("Searching memories for agent #{agent_id}: #{String.slice(search_query, 0, 50)}...")
  ...
  defp store_interaction(agent_id, user_query, response, _query_context) do
    case Anderson.MemoryOS.STM.DialoguePage.add_to_stm(agent_id, user_query, response) do
      {:ok, page} ->
  ...
  defp schedule_heat_update(agent_id) do
    ...
    case Anderson.MemoryOS.STM.DialoguePage
      ...
      {:ok, [page | _]} ->
        %{"record_pk" => page.id}
        |> Anderson.MemoryOS.Workers.CheckSTMCapacityWorker.new(queue: :memory)
        |> Oban.insert()
  ...
  defp get_lpm_statistics(agent_id) do
    ...
    total_system_entries = Anderson.MemoryOS.SystemMemory
    |> Ash.read!()
    ...
    _ -> %{level: :system, contributed_entries: 0, total_system_entries: 0, contribution_ratio: 0.0}
  end
  ...
  defp format_search_results(results, :lpm) when is_map(results) do
    knowledge = Enum.map(results[:knowledge_entries] || [], fn entry ->
      %{
        id: entry.id,
    ...
    traits = Enum.map(results[:traits] || [], fn trait ->
      %{
        id: trait.id,
