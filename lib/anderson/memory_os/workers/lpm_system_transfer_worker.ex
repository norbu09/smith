defmodule Anderson.MemoryOS.Workers.LpmSystemTransferWorker do
  @moduledoc """
  ...
  def perform(%Oban.Job{args: %{"knowledge_entry_id" => entry_id}}) do
    ...
    case Anderson.MemoryOS.LPM.KnowledgeBaseEntry.by_id(entry_id) do
      {:ok, entry} ->
  ...
  defp perform_system_memory_maintenance() do
    ...
    if entries_count > max_entries do
      ...
      Enum.each(evicted_entries, fn entry ->
        ...
        Anderson.MemoryOS.SystemMemory.destroy(entry.id)
      end)
