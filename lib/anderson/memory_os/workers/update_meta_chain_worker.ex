defmodule Anderson.MemoryOS.Workers.UpdateMetaChainWorker do
  ...
  defp extract_topics(page) do
    ...
    topics = if String.contains?(String.downcase(text), ["memory", "remember", "recall"]) do
      ["memory_management" | topics]
    else
      topics
    end

    topics = if String.contains?(String.downcase(text), ["help", "how", "what", "why"]) do
      ["information_seeking" | topics]
    else
      topics
    end

    topics = if String.contains?(String.downcase(text), ["task", "do", "complete", "finish"]) do
      ["task_oriented" | topics]
    else
      topics
    end

    topics = if String.contains?(String.downcase(text), ["conversation", "chat", "talk"]) do
  ...
  defp analyze_sentiment(page) do
    ...
    positive_words = ["good", "great", "excellent", "amazing", "wonderful", "perfect", "love", "like", "enjoy", "happy", "pleased"]
  ...
  defp generate_summary(page) do
    ...
    interaction_type = cond do
      String.contains?(String.downcase(page.query), ["?", "how", "what", "why", "when", "where"]) -> "question"
      String.contains?(String.downcase(page.query), ["please", "can you", "could you"]) -> "request"
      response_words > query_words * 2 -> "detailed_response"
      true -> "general_interaction"
    end
  ...
  defp calculate_complexity(page) do
    ...
    text = "#{page.query} #{page.response}"
    technical_terms = ["algorithm", "database", "system", "process", "implementation", "configuration"]
    technical_score = Enum.count(technical_terms, &String.contains?(String.downcase(text), &1)) * 0.2
  ...
  defp common_stopwords do
    [
      ...
      "find", "found", "use", "used", "work", "worked", "try", "tried", "help",
      "helped", "start", "started", "stop", "stopped", "run", "ran", "play",
      "played", "turn", "turned", "show", "showed", "move", "moved", "live",
      "lived", "feel", "felt", "put", "keep", "kept", "let", "mean", "meant"
    ]
  end
end
