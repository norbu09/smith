defmodule Smith.MemoryOS.Workers.UpdateHeatScoreWorker do
  @moduledoc """
  ...
  def perform(%Oban.Job{args: %{"record_pk" => segment_id}}) do
    ...
    case Smith.MemoryOS.MTM.DialogueSegment.by_id(segment_id) do
      {:ok, segment} ->
  ...
  defp evict_cold_segments(segments, eviction_count) do
    ...
    Enum.each(coldest_segments, fn segment ->
      ...
      Smith.MemoryOS.MTM.DialogueSegment.destroy(segment.id)
    end)
  ...
  defp get_agent_config(agent_id) do
    ...
    case Smith.MemoryOS.Configuration.get_or_create(agent_id, "default", %{}) do
      ...
      {:error, _} ->
        ...
        %{
          mtm_capacity: Application.get_env(:smith, :memory_os)[:default_mtm_capacity] || 200,
          heat_threshold: Application.get_env(:smith, :memory_os)[:default_heat_threshold] || 5.0
        }
    end
  end
end
