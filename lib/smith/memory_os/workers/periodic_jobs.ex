defmodule Smith.MemoryOS.Workers.PeriodicJobs do
  @moduledoc """
  ...
  def schedule_heat_updates(agent_id) do
    ...
    %{agent_id: agent_id}
    |> Smith.MemoryOS.Workers.UpdateHeatScoreWorker.new(
      # 1 hour
      schedule_in: 3600,
      queue: :memory
    )
  ...
  def schedule_lpm_evaluation(agent_id) do
    ...
    %{agent_id: agent_id}
    |> Smith.MemoryOS.Workers.LpmSystemTransferWorker.new(
      # 6 hours
      schedule_in: 21600,
      queue: :memory
    )
  ...
  def cron_config do
    [
      ...
      {"0 */2 * * *", Smith.MemoryOS.Workers.PeriodicJobs,
       args: %{task: "update_all_heat_scores"}},

      # Evaluate LPM for SystemMemory promotion every 6 hours
      {"0 */6 * * *", Smith.MemoryOS.Workers.PeriodicJobs,
       args: %{task: "evaluate_lpm_promotion"}},

      # System maintenance daily at 2 AM
      {"0 2 * * *", Smith.MemoryOS.Workers.PeriodicJobs, args: %{task: "system_maintenance"}},

      # Clean up old meta chains weekly
      {"0 3 * * 0", Smith.MemoryOS.Workers.PeriodicJobs, args: %{task: "cleanup_meta_chains"}}
    ]
  end
