defmodule Smith.MemoryOS.Workers.CheckSTMCapacityWorker do
  @moduledoc """
  ...
  def perform(%Oban.Job{args: %{"record_pk" => page_id}}) do
    ...
    case Smith.MemoryOS.STM.DialoguePage.by_id(page_id) do
      {:ok, page} ->
  ...
  defp transfer_page_to_mtm(page) do
    ...
    case best_segment do
      ...
      {:not_found, _reason} ->
        ...
        case Smith.MemoryOS.MTM.DialogueSegment.create_from_page(page.id) do
          {:ok, _segment} ->
            :ok
  ...
  defp get_agent_config(agent_id) do
    ...
    case Smith.MemoryOS.Configuration.get_or_create(agent_id, "default", %{}) do
      ...
      {:error, _} ->
        ...
        %{
          stm_capacity: Application.get_env(:smith, :memory_os)[:default_stm_capacity] || 7,
          mtm_fscore_threshold:
            Application.get_env(:smith, :memory_os)[:default_fscore_threshold] || 0.6
        }
    end
  end
end
